<template>
  <el-table-column label="操作" :width="props.width" fixed="right" align="center">
    <template #header>
      <slot name="header">
        <el-button v-if="props.save" @click="props.save()" text style="color: #57965c">
    ...
    <template #default="scope">
      ...
      <slot name="default">
        <el-button v-if="props.edit" @click="props.edit(scope.row)" text circle>
        ...
        <el-popconfirm v-if="props.del" title="确定删除吗？" @confirm="props.del(scope.row)">
          <template #reference>
      ...
      <slot name="after-default" :scope="scope"  />
    </template>
  </el-table-column>
</template>
